  ...
  try {
    ...
    try {
      ...
      uploadedDocs.push(cloudUrl);
        ...
        return res.status(500).json({ error: "Cloudinary upload crashed", detail: err.message });
    }
...
//         // Call ABBYY with local path (optional - could switch to Cloudinary URL if ABBYY supports it)
//         const result = await launchAbbyyTransaction(filePath, file.originalname);
//         abbyyResults.push(result);
