...
export const handleManualChat = (req, res) => {
    ...
    if (session.step === "awaiting_service") {
        ...
        return res.json({
            reply: "Please type 'card', 'loan', or 'account' to proceed.",
        });
        }
    }
    ...
    if (session.step === "awaiting_pan") {
        ...
        return res.json({
            reply: `Thank you! Your CIBIL score is ${session.cibil}. Here are your recommended ${session.service.replace("_", " ")} options:`,
            options: options,
        });
        } else {
        return res.json({
    ...
    return res.json({
        reply: "I'm not sure how to help. Please type 'card', 'loan', or 'account'.",
    });
};
